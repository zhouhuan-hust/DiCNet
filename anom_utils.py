...
def get_measures(_pos, _neg, recall_level=recall_level_default):
    ...
    fpr, thr = fpr_and_fdr_at_recall(labels, examples, recall_level)

    return auroc, aupr, fpr, thr
...
def get_and_print_results(out_score, in_score, num_to_avg=1):
    ...
    return auroc, aupr, fpr, measures[-1]

def eval_ood_measure(score_map, seg_label, out_label=13):
    # this code subtracts one
    # ano_mask = np.logical_or(seg_label==18, seg_label==19) 
    # in_scores = score_map[~ano_mask]
    # out_scores = score_map[ano_mask]
    in_scores = score_map[seg_label != out_label]
    out_scores  = score_map[seg_label == out_label]

    if (len(out_scores) != 0) and (len(in_scores) != 0):
        auroc, aupr, fpr, thr = get_and_print_results(out_scores, in_scores)
        return auroc, aupr, fpr, thr
    else:
        return None
