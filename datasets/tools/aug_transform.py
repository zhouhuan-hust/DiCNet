...
class RandomHFlip(object):
    ...
    def __call__(self, img, labelmap=None, maskmap=None, kpts=None, bboxes=None, labels=None, polygons=None):
        ...
        if random.random() > self.ratio:
            return img, labelmap, maskmap, kpts, bboxes, labels, polygons, False
        ...
        return img, labelmap, maskmap, kpts, bboxes, labels, polygons, True
...
class RandomResize(object):
    ...
    def get_scale(self, img_size, bboxes):
        if self.method == 'random':
            ...
            raise ValueError('Resize method {} is invalid.'.format(self.method))
...
class RandomCrop(object):
    ...
    def get_lefttop(self, crop_size, img_size):
        if self.method == 'center':
            ...
            Raise('Crop method {} is invalid.'.format(self.method))
...
class CV2AugCompose(object):
    ...
    def __init__(self, configer, split='train'):
        ...
        self.split = split
        self.aug_trans = getattr(configer.DATASET.aug_transform, self.split)

        for trans in self.aug_trans["aug_keys"]:
            self.transforms[trans] = CV2_AUGMENTATIONS_DICT[trans](**self.aug_trans[trans])

    def __call__(self, img, labelmap=None, maskmap=None, kpts=None, bboxes=None, labels=None, polygons=None):

        if self.configer.DATASET.input_mode == 'RGB':
            img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        
        flip_flag = False
        for trans_key in self.aug_trans["aug_keys"]:
            if trans_key == "random_hflip":
                (img, labelmap, maskmap, kpts,
             bboxes, labels, polygons, flip_flag) = self.transforms[trans_key](img, labelmap, maskmap,
