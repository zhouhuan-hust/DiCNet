...
class DefaultLoader(data.Dataset):
    def __init__(self, split, data_root="./data", aug_transform=DefaultAugTransform, 
        ...
        odgt = osp.join(data_root, "odgt_street", f"{split}.odgt")
    ...
    def __getitem__(self, index):
        ...
        if self.label_transform is not None:
            labelmap = self.label_transform(labelmap)[...,0] # x = np.random.rand(2, 3, 4) x[:, ：, 0] x[..., 0]

        maskmap = torch.ones_like(labelmap, dtype=torch.uint8)

        return {'x1': image1, 'x2': image2, 'labelmap': labelmap, 'maskmap': maskmap, "flip_flag": torch.as_tensor(flip_flag)}
...
if __name__ == "__main__":
    data_root = "./data"
    loader = data.DataLoader(DefaultLoader("train", data_root),
                            batch_size=4,
                            shuffle=False,
                            num_workers=1,
                            collate_fn=lambda batch: collate(batch, {"input_size": (700, 700),
                                                                    "pad_mode": "pad_center",
                                                                    "fea_stride": 8}))
