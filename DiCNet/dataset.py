...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def segm_transform(self, segm, wh=None):
        if wh is not None:
            segm.resize(wh, Image.NEAREST)
        # to tensor, -1 to 149
...
class TrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_per_gpu):
            ...
            batch_widths[i] = img_width 
            batch_heights[i] = img_height 
    ...
    def __len__(self):
        return self.num_sample // self.batch_per_gpu 
...
class ValDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # segm transform, to torch long tensor HxW
        segm = self.segm_transform(segm, (160, 90))
        batch_segms = torch.unsqueeze(segm, 0)

        output = dict()
        output['img_ori'] = np.array(img)
        output['img_data'] = img_resized[0]
...
if __name__ == "__main__":
    ...
    data_set = TrainDataset("data", "data/training.odgt", opt, batch_per_gpu=2)
    data_test = ValDataset("data", "data/test.odgt", opt)
    ...
    for batch in loader_test:
        print(batch['img_data'].shape)
        print(batch['seg_label'].shape)
        print(batch.keys())